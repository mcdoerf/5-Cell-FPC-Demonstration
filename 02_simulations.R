...
sim_study <- function(nsim = 1000, Ntot, N_flu,
  ...
  for (i in seq_len(nsim)) {
    ...
    inf <- inference(dat, psi=p2, alpha = alpha, postdraws=postdraws)
...
save(all_results1, file = "sim_results1b_arXiv.RData")
...
save(all_results2, file = "sim_results2b_arXiv.RData")
...
save(all_resultsA3, file = "sim_results_A3b_arXiv.RData")
...
save(all_resultsA4, file = "sim_results_A4b_arXiv.RData")




# 5th set (similar to CRISP) ----------------------------------------------


scenariosCRISP<- expand.grid(
  N_flu = c(156),
  p2 = c(200/1029),  ##200/1029
...
save(all_resultsCRISP, file = "sim_results_CRISPb_arXiv.RData")
...
build_index_map <- function(n_scenarios, N_len = 4, P_len = 3) {
...
# ---------------------------
make_full_table <- function(all_results1, N_vals, write_to = NULL) {
  N_vals  <- N_vals
  p2_vals <- c(0.1, 0.25, 0.5)
  ...
  out_lines <- c(out_lines, "\\begin{tabular}{ccccccc}")
  out_lines <- c(out_lines, "\\hline")
  out_lines <- c(out_lines, " & \\multicolumn{2}{c}{$\\psi=0.1$} & \\multicolumn{2}{c}{$\\psi=0.25$} & \\multicolumn{2}{c}{$\\psi=0.5$} \\\\")
  out_lines <- c(out_lines, "\\hline")
  out_lines <- c(out_lines, "Estimator & \\makecell{Mean (SD) \\\\ {[avg. SE]}} & \\makecell{CI coverage \\\\ {[avg. width]}} & \\makecell{Mean (SD) \\\\ {[avg. SE]}} & \\makecell{CI coverage \\\\ {[avg. width]}} & \\makecell{Mean (SD) \\\\ {[avg. SE]}} & \\makecell{CI coverage \\\\ {[avg. width]}} \\\\")
  out_lines <- c(out_lines, "\\hline")
  
  for (r in seq_along(N_vals)) {
    out_lines <- c(out_lines, sprintf("\\multicolumn{7}{c}{$N=%d$}\\\\", N_vals[r]))
    out_lines <- c(out_lines, "\\hline")
    ...
    five_cells <- lapply(seq_along(p2_vals), function(c) {
      ...
      if (is.na(idx)) {
        ...
        make_latex_cell(all_results1[[idx]],
                        ...
                        # ordering for the 2x2: Wald unadj | Wald FPC1
                        #                      Bayes unadj | Bayes FPC1
    ...
    out_lines <- c(out_lines, paste0("$\\hat{N}_5$ \\tnote{a} & ", row_five, " \\\\"))
    ...
    out_lines <- c(out_lines, paste0("$\\hat{N}_{RS}$ \\tnote{b} & ", row_rs, " \\\\"))
...
# Making Tables for simulation sets 1-4 ----------------------------------------------


#Removing simulation results where psi=0.05.  We considered this unrealistically low for the anchor stream sample.

all_results1_reduced<-all_results1[-c(1,2,3,4)]
all_results2_reduced<-all_results2[-c(1,2,3,4)]
all_resultsA3_reduced<-all_resultsA3[-c(1,2,3,4)]
all_resultsA4_reduced<-all_resultsA4[-c(1,2,3,4)]

make_full_table(all_results1_reduced, N_vals=c(500, 1000, 2500, 5000))
make_full_table(all_results2_reduced, N_vals=c(50, 100, 250, 500))
make_full_table(all_resultsA3_reduced, N_vals=c(13, 25, 63, 125))
make_full_table(all_resultsA4_reduced, N_vals=c(25, 50, 125, 250))



# Making tables for simulation set 5 -------------------------------------------------
...
make_latex_cell <- function(res, method,
  ...
  if (!is.null(ci_order) && length(ci_order) >= 1) {
    ...
    if (length(ci_order) == 6) {
      right <- sprintf("\\begin{tabular}{ccc}\n%s & %s & %s \\\\\n%s & %s & %s \\\\\n%s & %s &%s \\\\\n%s & %s & %s \n\\end{tabular}",
                       cov[1], cov[2], cov[3], wid[1], wid[2], wid[3], cov[4], cov[5], cov[6], wid[4], wid[5], wid[6])
    } else if (length(ci_order) == 4) {
      right <- sprintf("\\begin{tabular}{cc}\n%s & %s \\\\\n%s & %s \\\\\n%s & %s \\\\\n%s & %s\n\\end{tabular}",
                       cov[1], cov[2], wid[1], wid[2], cov[3], cov[4], wid[3], wid[4])
    } else if (length(ci_order) == 3) {
      right <- sprintf("\\begin{tabular}{cc}\n%s & %s \\\\\n%s & %s \\\\\n & %s \\\\\n & %s\n\\end{tabular}",
...
# Build mapping matrix from list-index -> (psymp_flu_index, p1_symp_index)
build_index_map <- function(n_scenarios, F_len = 4, P_len = 3) {
  map <- matrix(NA_integer_, nrow = F_len, ncol = P_len)
  for (i in seq_len(n_scenarios)) {
    k <- (i-1)%%F_len + 1
    F_idx  <- k
    P_idx  <- ceiling(i/F_len)
    map[F_idx, P_idx] <- i
  }
  map
}
...
# Main: create full table
# ---------------------------
make_full_table <- function(all_results1, scenarios1, write_to = NULL) {
  flu_vals <- unique(scenarios1$psymp_flu)
  p1_vals  <- unique(scenarios1$p1_symp)
  n_scen   <- length(all_results1)
  
  # build index map
  map_idx <- build_index_map(n_scen, F_len = length(flu_vals), P_len = length(p1_vals))
  
  out_lines <- character(0)
  # use paste0 instead of %+%
  out_lines <- c(out_lines,
                 paste0("\\begin{tabular}{c|",
                        paste(rep("c|c", length(p1_vals)), collapse="|"),
                        "|}"))
  out_lines <- c(out_lines, "\\hline")
  # Header row with p1 values
  header <- paste(sprintf("\\multicolumn{2}{c}{$p_{1|diseased}=%.2f$}", p1_vals), collapse=" & ")
  out_lines <- c(out_lines, paste("&", header, "\\\\"))
  out_lines <- c(out_lines, "\\hline")
  
  # Second header row
  inner_headers <- paste(rep("\\makecell{Mean (SD) \\\\ {[avg. SE]}} & \\makecell{CI coverage \\\\ {[avg. width]}}",
                             length(p1_vals)), collapse=" & ")
  out_lines <- c(out_lines, paste("Estimator &", inner_headers, "\\\\"))
  out_lines <- c(out_lines, "\\hline")
  
  for (r in seq_along(flu_vals)) {
    out_lines <- c(out_lines, sprintf("\\multicolumn{%d}{c}{$p_{symp|diseased}=%.2f$}\\\\", 2*length(p1_vals)+1, flu_vals[r]))
    out_lines <- c(out_lines, "\\hline")
    
    # five-cell row
    five_cells <- lapply(seq_along(p1_vals), function(c) {
      idx <- map_idx[r, c]
      if (is.na(idx)) {
        list(left="--", right="--")
      } else {
    ...
    # RS row
    rs_cells <- lapply(seq_along(p1_vals), function(c) {
      idx <- map_idx[r, c]
      if (is.na(idx)) {
        list(left="--", right="--")
      } else {
    ...
    # Chapman row
    chap_cells <- lapply(seq_along(p1_vals), function(c) {
      idx <- map_idx[r, c]
      if (is.na(idx)) {
        list(left="--", right="--")
      } else {
...
make_full_table(all_resultsCRISP, scenariosCRISP)
